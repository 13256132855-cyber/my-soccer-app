...
with st.write("【让球胜平负】")
    ...
    # 🌟 1. 放在最外层，绝不报错的让球数输入框
    handicap_val = st.number_input("设置让球数 (如让2球填 -2, 让1球填 -1, 受让1球填 1)", min_value=-10, max_value=10, value=-1, step=1)
    ...
    # 🌟 3. 生成 3 个并排的输入框，直接使用动态标签
    col_rq1, col_rq2, col_rq3 = st.columns(3)
    rq_s = col_rq1.number_input(f"{h_label}-胜", 1.0, 1000.0, 1.82, 0.01)
    rq_p = col_rq2.number_input(f"{h_label}-平", 1.0, 1000.0, 3.40, 0.01)
    rq_f = col_rq3.number_input(f"{h_label}-负", 1.0, 1000.0, 3.50, 0.01)
